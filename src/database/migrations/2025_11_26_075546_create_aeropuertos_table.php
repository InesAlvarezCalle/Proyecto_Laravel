<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('aeropuertos', function (Blueprint $table) {
            $table->id();
            $table->string('nombre', 255)->notNullable();
            $table->string('codigo', 255)->notNullable();
            $table->string('ciudad', 255)->notNullable();
            $table->enum('tipo', ['Internacional', 'Domestico', 'Regional'])->default('Internacional')->notNullable();
            $table->decimal('latitud')->notNullable();
            $table->decimal('longitud')->notNullable();
            $table->integer('elevacion')->notNullable();
            $table->integer('terminal')->notNullable();
            $table->integer('anyo_apertura')->notNullable();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('aeropuertos');
    }
};
